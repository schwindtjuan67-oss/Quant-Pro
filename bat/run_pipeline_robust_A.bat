@echo off
setlocal ENABLEDELAYEDEXPANSION

REM =====================================
REM MOVERSE AL ROOT DEL REPO
REM (si este .bat vive en /scripts, esto sube 1 nivel)
REM =====================================
cd /d "%~dp0\.."

REM =====================================================
REM CONFIG GENERAL
REM =====================================================

set PYTHON=python

REM --- Scripts ---
set ROBUST_SCRIPT=analysis\robust_optimizer.py
set POST_SCRIPT=analysis\analysis_post_robust.py

REM --- Data / Config ---
set DATA=datasets\SOLUSDT\1m
set BASE_CFG=configs\pipeline_research_backtest.json

REM --- Outputs ---
set ROBUST_OUT_DIR=results\robust
set PROMO_OUT_DIR=results\promotions

REM =====================================================
REM HARDWARE SAFE (8 GB RAM)
REM =====================================================

set WORKERS=3
set BATCH_SIZE=4
set SAMPLES=300

REM Evitar oversubscription de BLAS dentro de multiproceso
set OMP_NUM_THREADS=1
set MKL_NUM_THREADS=1
set OPENBLAS_NUM_THREADS=1
set NUMEXPR_MAX_THREADS=1

REM =====================================================
REM SEEDS A ROTAR
REM =====================================================

set SEEDS=1337 2024 777

REM =====================================================
REM VENTANAS TEMPORALES (FASE A)
REM =====================================================

set WINDOWS=2019-01_2020-12 2021-01_2021-12 2022-01_2023-06

REM =====================================================
REM LOOP CONTROL (sin intervención)
REM  - MAX_CYCLES=0 => infinito
REM  - STOP_ON_PROMOTION=0 => sigue explorando aunque haya promoción
REM =====================================================

set MAX_CYCLES=0
set STOP_ON_PROMOTION=0
set SLEEP_BETWEEN_CYCLES_SEC=10

REM =====================================================
REM SANITY CHECK (CRÍTICO)
REM =====================================================

echo =========================================
echo [SANITY]
echo DATA=%DATA%
echo BASE_CFG=%BASE_CFG%
echo WORKERS=%WORKERS%  BATCH_SIZE=%BATCH_SIZE%  SAMPLES=%SAMPLES%
echo SEEDS=%SEEDS%
echo WINDOWS=%WINDOWS%
echo =========================================
echo.

REM =====================================================
REM CREAR DIRECTORIOS
REM =====================================================

if not exist "%ROBUST_OUT_DIR%" mkdir "%ROBUST_OUT_DIR%"
if not exist "%PROMO_OUT_DIR%" mkdir "%PROMO_OUT_DIR%"

REM =====================================================
REM LOOP PRINCIPAL
REM =====================================================

set /a CYCLE=0

:LOOP
set /a CYCLE+=1

REM Timestamp para evitar overwrites (PowerShell nativo en Windows)
for /f %%T in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMdd_HHmmss"') do set TS=%%T

echo =========================================
echo [PIPELINE] CYCLE=!CYCLE!  TS=!TS!
echo =========================================

REM =====================================================
REM FASE A — ROBUST OPTIMIZATION
REM =====================================================

for %%W in (%WINDOWS%) do (
    for %%S in (%SEEDS%) do (

        set OUT_FILE=%ROBUST_OUT_DIR%\robust_%%W_seed%%S_!TS!.json

        echo -----------------------------------------
        echo [ROBUST] WINDOW=%%W  SEED=%%S
        echo [ROBUST] OUT=!OUT_FILE!
        echo -----------------------------------------

        %PYTHON% %ROBUST_SCRIPT% ^
            --data "%DATA%" ^
            --base-config "%BASE_CFG%" ^
            --window %%W ^
            --samples %SAMPLES% ^
            --seed %%S ^
            --workers %WORKERS% ^
            --batch-size %BATCH_SIZE% ^
            --parallel ^
            --out "!OUT_FILE!"

        if errorlevel 1 (
            echo [ERROR] Robust optimizer failed (WINDOW=%%W SEED=%%S). Continuing...
            echo.
        ) else (
            echo [ROBUST] Finished %%W seed %%S
            echo.
        )
    )
)

echo =========================================
echo [PIPELINE] FASE A sweep done (cycle=!CYCLE!)
echo =========================================

REM =====================================================
REM POST-ANALYSIS + AUTO-PROMOTION (A → B)
REM =====================================================

echo =========================================
echo [POST] Running analysis_post_robust.py
echo =========================================

%PYTHON% %POST_SCRIPT%

REM =====================================================
REM CHECK PROMOTION RESULT
REM =====================================================

set PROMOTED_FILE=%PROMO_OUT_DIR%\faseA_promoted.json

if exist "%PROMOTED_FILE%" (
    for %%A in ("%PROMOTED_FILE%") do (
        if %%~zA GTR 40 (
            echo =========================================
            echo [PROMOTER] PROMOTION SUCCESS (faseA_promoted.json created)
            echo =========================================
            if "%STOP_ON_PROMOTION%"=="1" goto END
        )
    )
) else (
    echo [PROMOTER] No promotion in this cycle.
)

REM =====================================================
REM LOOP STOP CONDITIONS
REM =====================================================

if not "%MAX_CYCLES%"=="0" (
    if !CYCLE! GEQ %MAX_CYCLES% goto END
)

echo [PIPELINE] Sleeping %SLEEP_BETWEEN_CYCLES_SEC%s then continuing...
timeout /t %SLEEP_BETWEEN_CYCLES_SEC% /nobreak >nul
goto LOOP

:END
echo =========================================
echo [PIPELINE] FINISHED
echo =========================================
pause
